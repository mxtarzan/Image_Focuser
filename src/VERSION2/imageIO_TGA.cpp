...
ImageStruct* readTGA(const char* filePath)
{
	...
	unsigned int imgWidth = (int)(((unsigned int)head[12]&0xFF) | (unsigned int)head[13]*256);
	unsigned int imgHeight = (int)(((unsigned int)head[14]&0xFF) | (unsigned int)head[15]*256);
	...
	ImageStruct* image = new ImageStruct(imgWidth, imgHeight, imgType, 1);
	...
	if(image->type == RGBA32_RASTER)
	{
		...
		if(head[17]&0x20)
		{
			unsigned char* ptr = data + imgSize*4 - image->bytesPerRow;
	...
	else
	{
		...
		if(head[17]&0x20)
		{
			unsigned char* ptr = data + imgSize - image->width;
			for(unsigned int i = 0; i < image->height; i++)
			{
...
//----------------------------------------------------------------------*/ 
ImageIOErrorCode writeTGA(const char* filePath, const ImageStruct* image)
{
	//--------------------------------
